...
또한 useController의 rules 옵션으로 유효성 검사도 처리할 수 있기 때문에, 굳이 register를 병행하지 않아도 된다. 이러한 이유로 AddressInput 컴포넌트에서 register 관련 코드를 제거하고, useController만으로 필드 등록과 유효성 검사를 관리하도록 수정했다!<br/>
해당 내용은 [react-hook-form 공식문서](https://react-hook-form.com/docs/usecontroller)에 나와있다.
더 구체적으로는 아래의 캡쳐본을 보자!<br/>
