...
`useController`는 react-hook-form과 컴포넌트를 바인딩하면서도 `value`, `onChange`, `error` 같은 속성을 직접 제어할 수 있다.
...
이전에는 register와 useController를 동시에 사용했는데, 이는 같은 필드를 중복 등록하게 되어 충돌이 발생할 수 있다.
[공식 문서](https://react-hook-form.com/docs/usecontroller)에 따르면 useController는 내부적으로 register를 호출하므로 **둘 중 하나만 사용해야 한다**고 나와있다.

더 구체적으로는 아래의 캡쳐본을 보자!

![register 캡쳐본](/images/posts/Dev/international-address/register_캡쳐.png)

이에 따라 AddressInput에서는 register 관련 코드를 제거하고, useController만으로 필드 등록과 유효성 검사를 관리하도록 수정했다.
또한 rules 옵션으로 `required`, `pattern`, `maxLength` 등을 처리할 수 있어 별도의 register 호출이 필요하지 않다. (ex. 중국 신분증 번호 정규식 `/^[0-9]{17}[0-9Xx]$`/을 rules에 직접 지정)
이러한 이유로 아래처럼 register 코드를 제거하고, useController만으로 필드 등록과 유효성 검사를 관리하도록 수정했다!  
```
